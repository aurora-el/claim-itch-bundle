...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client
        .form(Locator::Css(".login_form_widget > form"))
        .await?
    ...
    // client.wait().for_element(
    //     Locator::Css("[data-page_name='dashboard'],\
    //         [data-page_name='my_feed']")).await?;
    client
        .wait()
        .for_element(Locator::Css(
            "[data-page_name]:not([data-page_name='user_login'])",
        ))
        .await?;
    ...
    for bundle in &args.bundle {
        ...
        loop {
            ...
            while let Ok(game) = client.find(Locator::Css("button[value=claim]")).await {
                ...
                match client
                    .find(Locator::Css(".game_download_header_widget .object_title"))
                    .await
                {
                    Ok(title) => {
                        // log the game's name
                        println!("{:?} claimed", title.text().await?);
                    }
                    Err(_) => {
                        let url = client.current_url().await?;
                        eprintln!("something went wrong while claiming {:?}", url.to_string())
                    }
                }
                client.back().await?;
            }
